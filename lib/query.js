 ...
 * unset param
 *
 * @param {String} fieldname - param fieldname
 *
 * @returns {Query}
 */
Query.prototype.unsetParam = function (fieldname) {
  const filterRegex = new RegExp("^" + fieldname + "=")
  this.params = this.params.filter((param) => {
    // remove field from params
    return !param.match(filterRegex)
  })
  return this
}
 ...
 *
 * @param {Object[]} param
 * @param {String} param.field - param field
 * @param {String} param.value - param value
 *
 * @returns {Query}
 */
Query.prototype.setParam = function (param) {
  return this.unsetParam(param.field)
             .addParams([param])
};
